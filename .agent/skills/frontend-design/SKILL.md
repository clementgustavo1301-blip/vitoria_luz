...
├── **Aspect Ratio**: Use `aspect-ratio` to prevent layout shifts (CLS).
└── **Grid Areas**: Use named areas for complex asymmetric layouts.
```
...
### Fluid Typography
├── Use `clamp(min, preferred, max)` for seamless scaling without media queries.
├── Base line-height: 1.5 for accessibility.
└── Letter-spacing: tighter for large display headings (-0.02em).
```
...
### Modern Motion
├── **View Transitions API**: Use for seamless page-level morphing.
├── **Staggered Reveals**: Entrance animations should be staggered (50ms offset).
└── **Layout Animations**: Use `framer-motion` or CSS `transition` for size changes.
...
- [ ] **Performance (LCP < 2.5s)**: No heavy unused libraries.
- [ ] **Fluidity**: No layout shifts during loading.
